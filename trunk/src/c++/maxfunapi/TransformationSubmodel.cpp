...
const double       LAMBDA_ONE_DEFAULT_UB    =  MF_INFINITY;
                              ...
                              double lambda1_ub)
{
  ...
  my_parameters[0] = ParameterInput("Transformation", "lambda_one", Parameter::INDEPENDENT,
                                        LAMBDA_ONE_DEFAULT_VALUE, 
                                        LAMBDA_ONE_DEFAULT_LB,
                                        LAMBDA_ONE_DEFAULT_UB);

  my_parameters[1] = ParameterInput("Transformation", "lambda_two", Parameter::FIXED, 
                                         LAMBDA_TWO_DEFAULT_VALUE, -MF_INFINITY, MF_INFINITY);
...
TransformationSubmodel::set_lambda_one(const model_input& lambda1)
{
  ...
  if(! SAGE::isnan(lambda1.value))
  {
    if(! lambda1.fixed)
    {
      ...
      my_parameters[0].initial_type     = Parameter::INDEPENDENT;
    ...
    {
      my_parameters[0].initial_estimate = lambda1.value;
      my_parameters[0].initial_type     = lambda1.fixed ? Parameter::FIXED : Parameter::INDEPENDENT;
...
TransformationSubmodel::adjust_lambda_one()
{
  ...
  assert(my_parameters[0].initial_type != Parameter::FIXED);
...
TransformationSubmodel::set_lambda_two(const model_input& lambda2)
{
  if(! SAGE::isnan(lambda2.value))
  {
    ...
    my_parameters[1].initial_type     = lambda2.fixed ? Parameter::FIXED : Parameter::INDEPENDENT;
  }
