...
int main(int argc, char* argv[])
{
  // 1. Fetch method type:
	...
	MAXFUN::max_method method;

	if(argc > 1)
	  imethod = atoi(argv[1]);

	if(imethod <= 0 || imethod > 6)
	  imethod = 1;

	method = (MAXFUN::max_method)imethod;
  ...
  // 3. Set up MaxfunConfig:

	MAXFUN::MaxfunConfig myconfig(MAXFUN::max_user);

	myconfig.AddMaxfunRun(method, 1000);
	myconfig.GetLatestMaxfunRun().epsilon1 = 1e-7;
	myconfig.GetLatestMaxfunRun().var_cov  = MAXFUN::final;

	myconfig.DuplicateLatestMaxfunRun();
	myconfig.GetLatestMaxfunRun().max_iterations = 0;

  // 4. Maximize

	Maximize(myconfig, myfun.minfo, myfun, true);
